...
def ik_soln_exists(goal_pose, kin, use_grasp_bias=False, use_rr=True):
    """
    ...
    :param use_grasp_bias: whether to bias the solution goal angles to a random set of grasp angles
    ...
    if goal_pose is not None:
        goal_angles = None
        if goal_pose:
            goal_angles = kin.solve(pose=goal_pose, use_rr_grasp_bias=use_grasp_bias, use_rr=use_rr)
            if goal_angles is not None and goal_angles.all():
                goal_angles = goal_angles.reshape(1, 7)
...
class RRT:
    ...
    def add_new_node(self, q_new, curr_dist_to_goal):
        self.cleanup_nodes()
        q = q_new
        if (type(q) == np.matrix or type(q) == np.ndarray) and q.shape[0] == 1 and q.shape[1] >= 1:
            q = q.tolist()[0]
        self.nodes.append(q)
        self.pos_nodes.append((curr_dist_to_goal, self.fwd_kin(q)))
    ...
    def extend_toward_goal(self, dist_thresh=0.02, use_pinv=True, use_adv_gradient_descent=True, time_limit=10):
        """
    ...
    def generate_random_goal_point(self, curr_pos, curr_diameter):
        ...
        for i in range(3):
            ...
            if diff == 0:
                ...
                continue
            offset = math.fabs(goal_coord - curr_coord) + curr_diameter/2.
    ...
    def add_point_if_valid(self, next_point, avoidance_diameter, goal_pose, prev_dist_to_goal):
    ...
    def ik_extend_randomly(self, curr_pos, dist_thresh, avoidance_diameter=0.2, num_tries=5):
        """
